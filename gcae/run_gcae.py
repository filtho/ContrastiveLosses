...
class Autoencoder(Model):
	...
	def call(self, input_data, targets=None, is_training = True, verbose = False, rander=[False, False], regloss=True, encode_only= False):
		'''
		...
		if self.regularizer and regloss and encoded_data is not None:
			...
			# Thought, how to translate this when using recombined samples...?

			#def fun_reg_fun(encoded_data_pure):

			"""
			# Pure box-contstaints:
			reg_loss = self.regularizer["reg_factor"] * tf.reduce_sum(tf.math.maximum(0., tf.square(encoded_data_pure) - 1 * 40000.))
			reg_loss = self.regularizer["reg_factor"] * tf.reduce_sum(tf.math.maximum(0., tf.square(encoded_data_pure) - 1 * 100))

			# Playing around
			lim = np.sqrt(100)
			reg_loss = self.regularizer["reg_factor"] * tf.reduce_sum(tf.math.maximum(0., tf.square(encoded_data_pure) - 1 * 100)) + 0.01 -0.01 * tf.math.exp(-1/2 * ((encoded_data_pure[:,0]-5)**2 +encoded_data_pure[:,1]**2) )-0.01 * tf.math.exp(-1/2 * ((encoded_data_pure[:,0]+5)**2 +encoded_data_pure[:,1]**2) )
			period = 10
			reg_loss = self.regularizer["reg_factor"] * (tf.reduce_sum(tf.math.maximum(0., tf.square(encoded_data_pure) - 1 * 100)) + tf.reduce_sum(50 * (-0.1+tf.math.maximum(0.1,tf.math.sin((encoded_data_pure[:,0] * (2*math.pi/period))) * tf.math.sin((encoded_data_pure[:,1] * (2*math.pi/period))))))) #  / tf.math.abs(tf.math.sin((encoded_data_pure[:,0] * (2*math.pi/10))) * tf.math.sin((encoded_data_pure[:,1] * (2*math.pi/10)))   )  ))

			#tf.debugging.enable_check_numerics()

			#tf.print(tf.reduce_min(0.001 + tf.cast((encoded_data_pure[:,0,tf.newaxis] - tf.transpose(encoded_data_pure[:,0,tf.newaxis]) )**2 + (encoded_data_pure[:,0,tf.newaxis] - tf.transpose(encoded_data_pure[:,0,tf.newaxis]))**2,tf.float32)))
			# Needed to add something small here, since otherwise we get nan values
			D = tf.math.sqrt(0.001 + (encoded_data_pure[:,0,tf.newaxis] - tf.transpose(encoded_data_pure[:,0,tf.newaxis]) )**2 + (encoded_data_pure[:,0,tf.newaxis] - tf.transpose(encoded_data_pure[:,0,tf.newaxis]))**2)

			#tf.print(tf.reduce_min(D))

			D = tf.cast(D,tf.float32)
			Dstar = D + tf.cast(tf.eye(len(D)) * 2 * lim * tf.math.sqrt(2.),tf.float32)
			sigma = 5   # 0.8 # lim/2./(np.sqrt(2.)*3.) # lim**2 / 2000 / 3
			dist_penalty = 0. #  ((lim+5)**2 - lim**2 )* 1* tf.reduce_mean(tf.math.exp(-1/2 * (Dstar/sigma)**2))

			#(1. / D) * (tf.ones_like(D,tf.float32) -tf.eye(len(D)))
			#DI = 1./(D+tf.eye(len(D))*2*lim*tf.math.sqrt(2.))
			"""
			if self.regularizer:
			...
			#print(encoded_data_pure.shape)


			#exit()
			#if "num_samples" in self.regularizer  :
			#	reg_loss = self.regularizer["reg_factor"] * tf.reduce_sum(
				#	tf.math.maximum(0., tf.square(encoded_data_pure) - 1 * self.regularizer["num_samples"]))
...
def main():
	...
	if "SLURMD_NODENAME" in os.environ:
		...
		if num_workers > 1  and  not arguments["evaluate"]:
			...
			#strategy = tf.distribute.MultiWorkerMirroredStrategy(cluster_resolver=tf.distribute.cluster_resolver.SlurmClusterResolver(),
			#														communication_options=tf.distribute.experimental.CommunicationOptions(
			...
			# used these to check that each proc only has 1 gpu each, and that they do not share GPUS.
			#cluster_resolver=tf.distribute.cluster_resolver.TFConfigClusterResolver()
			#print(cluster_resolver.num_accelerators(task_type="worker", task_id=int(os.environ["SLURM_PROCID"])))
			#print(cluster_resolver.num_accelerators(task_type="worker", task_id=0))
			#print(cluster_resolver.num_accelerators(task_type="worker", task_id=1))
	...
	if (arguments['evaluate'] or arguments['animate'] or arguments['plot']):
		...
		ds_validation = data.create_dataset(chunk_size, "validation", shuffle = False)
		...
		for batch_dist_input, batch_dist_target, poplist in ds_validation:
		...
		with strategy.scope():
			...
			if not contrastive:
				if loss_class == tf.keras.losses.CategoricalCrossentropy or loss_class == tf.keras.losses.KLDivergence :
					# MOve this outside this function, and just use coeeff
					beta = 0.999
			...
			@tf.function
			def distributed_train_step(model, model2, optimizer, optimizer2, loss_function, input, targets, pure, phenomodel=None, phenotargets=None,data=None):

				per_replica_losses, local_num_total_k_mer, local_num_correct_k_mer, local_class_conc  = strategy.run(run_optimization, args=(model, model2, optimizer, optimizer2, loss_function, input, targets, pure, phenomodel, phenotargets,data))
			...
			def valid_batch(autoencoder, loss_func, input_valid_batch, targets_valid_batch,data):
				...
				#tf.print(encoded_data_valid_batch)
				if contrastive:
					...
					class_conc = compute_class_concordance(input_valid_batch, targets_valid_batch, output_valid_batch, data)
			...
			def proj_batch(model, loss_func, input_valid_batch, targets_valid_batch,data,poplist,poplist_batch):
				...
				if contrastive: 
					...
					num_total_k_mer, num_correct_k_mer =  compute_k_mer_concordance(input_valid_batch, targets_valid_batch, output, data)
					class_conc = compute_class_concordance(input_valid_batch, targets_valid_batch, output,data)

				try:
					# This wont work if first dimension is empty
					size = tf.shape(poplist_batch)[0]
			...
			@tf.function
			def compute_class_concordance(input,truth,prediction,data):
				"""
			...
			def compute_k_mer_concordance(input, truth, prediction, data):
				...
				for k in k_vec:
					...
					try:
						truth2 = tf.transpose(tf.concat([tf.cast(truth, tf.float32), -1.0 * tf.ones(shape=[tf.shape(truth)[0], 1])], axis=1))
			...
			def run_optimization_pareto(model, model2, optimizer, optimizer2, loss_function, input, targets, pure, phenomodel=None, phenotargets=None,data = None):
				...
				if contrastive:
					...
					class_conc = compute_class_concordance(input, targets, output, data)
	...
	if arguments['train']:
		...
		if "lr_scheme" in train_opts.keys():
			...
			#TODO
			# Here, also update the t_mul and m_mul for cosine decay. I.e., need to update the "first_decay_steps" to something new.
		...
		if use_new:
			...
			#ds = data.create_dataset_tf_record(chunk_size, "training",n_workers = num_workers) #, device_id = device_id) This uses manual sharding
		...
		#ds = data.create_dataset(chunk_size, "training")
		#ds = data.create_dataset_tf_record(chunk_size, "training",n_workers = num_workers)

		"""
		ds = data.create_dataset_tf_record(chunk_size, "training",n_workers = num_workers) #, device_id = device_id)
		print("Training ------------------------------")
		for i ,j,k,_,_ in ds:
			tf.print(tf.shape(i), tf.shape(j))
		ds = data.create_dataset_tf_record(chunk_size, "validation",n_workers = num_workers) #, device_id = device_id)
		print("Validation ------------------------------")
		for i ,j,k,_,_ in ds:
			tf.print(tf.shape(i), tf.shape(j))
		ds = data.create_dataset_tf_record(chunk_size, "project",n_workers = num_workers) #, device_id = device_id)
		print("Project ------------------------------")
		for i ,j,k,_,_ in ds:
			tf.print(tf.shape(i), tf.shape(j))
		...
		#dds_validation = ds
		##### TEST
		regularizer["num_samples"] = data.n_train_samples
		###
		...
		with strategy.scope():
			...
			if use_new:
				...
				input_test, targets_test, _,_,_ = next(ds.as_numpy_iterator())
				pass
			else:
	...
	if arguments['project']:
		...
		if use_new:
			...
			def dataset_fn(input_context):
				...
				return data.create_dataset_tf_record(chunk_size, "project", n_workers=num_workers,
													 shuffle=True)  # , device_id = device_id)
		...
		def compute_and_save_binned_conc(pred,true,name):
			...
			missing_val = float(norm_opts["missing_val"])
		...
		for epoch in epochs:
			...
			if isChief:
				...
				encoded_proj_valid = np.empty((0, n_latent_dim))
				encoded_proj_train = np.empty((0, n_latent_dim))
