...
class alt_data_generator(data_generator_ae):
    ...
    def generator(self, pref_chunk_size, training, shuffle=True):
        ...
        while chunks_read < num_chunks:
            ...
            if self.recombination and training:
                ...
                # Here, the parent indices may be more than the training set size. That could be okay.
                # Meaning one parent can have several offspring. What is not okay, or at least wasteful, is if a parent
                # mates with itself - giving identical offspring. This does not add any value.

                par_inds_diff = par_inds[1:] - par_inds[:-1]

                par_inds_fix = tf.random.uniform(
                    (tf.cast(tf.shape(tf.where(par_inds_diff == 0))[0], tf.int32),), minval=0,
                    maxval=n_samples - 1, dtype=tf.dtypes.int32)
                a = tf.sparse.SparseTensor(indices=tf.where(par_inds_diff == 0),
                                           values=par_inds_fix,
                                           dense_shape=(
                                            tf.cast(self.n_parent_samples() * batches_per_chunk, tf.int32),))

                b = tf.sparse.add(par_inds, a)
                par_inds_fixed = tf.math.floormod(b, n_samples)

                """
                # This following snippet shows the number of identical adjacent parents, and the fix.

                c = 0
                for i in par_inds_diff:
                    if i == 0:
                        c += 1
                tf.print("num identical adjacent parent samples:", c)

                c = 0
                par_inds_diff2 = par_inds_fixed[1:] - par_inds_fixed[:-1]
                for i in par_inds_diff2:
                    if i == 0:
                        c += 1
                tf.print("num identical adjacent parent samples after 1 fix:", c)
                """

                parent_inds = tf.gather(indices,
                                        indices=par_inds_fixed)
    ...
    def recombination_mapping(self, x, inds, args):
        ...
        n_recombined_samples_per_generation = tf.cast(
            tf.math.ceil((self.total_batch_size - self.batch_size) / self.generations), tf.int32)
        # Extract the original training samples, and the parent samples to be used in recombination
        ...
        artificial_superpop = tf.reshape(tf.convert_to_tensor(["z_art_Generated0"] * num_recombined_sample_this_generation),[num_recombined_sample_this_generation, 1])

        new_sample_subpops = tf.reshape(tf.convert_to_tensor(["Artificial_{}x".format(self.generations - self.generations_left + 1) for i in
                        range(num_recombined_sample_this_generation)]),[num_recombined_sample_this_generation, 1])
    ...
    def sparse_mapping(self, x, inds, last_batch):
        ...
        if self.only_recomb and self.training and self.recombination_rate != 0:
