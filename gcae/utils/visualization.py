...
def plot_coords_by_superpop(pop_coords_dict, outfileprefix, pop_superpop_file, savefig=True, plot_legend=True, epoch = "", box_area = 0):
	'''
	Plot the first two dimensions of the given data in a scatter plot.
...
def plot_clusters_by_superpop(pop_coords_dict, outfileprefix, pop_superpop_file, savefig=True, write_legend=False):
	...
	for spop in range(len(superpops)):
		...
		if len(this_pops) > 0:
			...
			plt.subplot(n_superpops, 1 ,  spop+1)
...
def train_test_KDE_plot(outfile_prefix,x_train, y_train,x_valid,y_valid,markersize):
	...
	fig = plt.figure(figsize=(fsize*1.2, fsize*1.2))

	ax = fig.add_axes(rect_scatter)
	ax_histx = fig.add_axes(rect_histx, sharex=ax)
	ax_histy = fig.add_axes(rect_histy, sharey=ax)
	if len(x_valid) > len(x_train):

		ax.plot(x_valid, y_valid, "b1", markersize = markersize,label = "Validation")
		ax.plot(x_train, y_train, "r2", markersize = markersize,label = "Training")
	else:
		ax.plot(x_train, y_train, "r2", markersize = markersize,label = "Training")
		ax.plot(x_valid, y_valid, "b1", markersize = markersize,label = "Validation")

	train = ["train"] * len(x_train)
	test = ["test"] * len(x_valid)

	train_test = train+test

	data_x = np.append(x_train,x_valid)
	data_y = np.append(y_train,y_valid)

	d= {'train_test': train_test ,'x':data_x,'y':data_y}
	df = pd.DataFrame(data=d)

	sns.kdeplot(data =df, x = "x", hue = "train_test", ax=ax_histx, palette = ['r', 'b'],common_norm = False, legend = False,)
	sns.kdeplot(data =df, y = "y", hue = "train_test", ax=ax_histy, palette = ['r', 'b'],common_norm = False, legend = False,)
